...
public class CompressImageUtil{
	...
	public void compress(String imagePath, String newImgPath, CompressListener listener) {
		try {
			compressImageByPixel(imagePath, newImgPath, listener);
		} catch (FileNotFoundException e) {
			listener.onCompressFailed(imagePath,String.format("图片压缩失败,%s",e.toString()));
			e.printStackTrace();
		}
...
//				}
	...
	private void compressImageByPixel(String imgPath, String newImgPath, CompressListener listener) throws FileNotFoundException {
		if(imgPath==null){
			sendMsg(false,imgPath,"要压缩的文件不存在",listener);
			return;
		}
		...
		bitmap.compress(Bitmap.CompressFormat.JPEG, 100, new FileOutputStream(new File(newImgPath)));
		listener.onCompressSuccess(newImgPath);
	}
	/**
	 * 发送压缩结果的消息
